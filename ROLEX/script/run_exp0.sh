...
base_dir="$HOME/skipvecdmecdm/ROLEX"
ae_data_dir="$HOME/skipvecdmecdm/AE/Data"
...
for dis in $distribution; do
    for thread in $threads; do
        for file_name in $workloads; do
            ...
            for n in $node; do
                sleep 2
                ssh w$n "cd $base_dir/build && nohup numactl --interleave=all ./ycsbc $thread 4 $file_name $dis > $base_dir/data/node$n-exp0_rolex_$file_name-$dis-thread$thread-coro4.txt 2>&1" &
            done

            sleep 2
            cd $base_dir/build && numactl --interleave=all ./ycsbc $thread 4 $file_name $dis \
                > $base_dir/data/node$driver-exp0_rolex_$file_name-$dis-thread$thread-coro4.txt 2>&1
            ...
            for n in $node; do
                scp "w$n:$base_dir/data/node$n-exp0_rolex_$file_name-$dis-thread$thread-coro4.txt" "$ae_data_dir/"
            done
            cp "$base_dir/data/node$driver-exp0_rolex_$file_name-$dis-thread$thread-coro4.txt" "$ae_data_dir/"
            if [ "$file_name" = "scan-only" ]; then
                for n in $node; do ssh w$n "rm -f $base_dir/build/load_keys.data"; done
                rm -f "$base_dir/build/load_keys.data"
            fi
...
echo "All ROLEX exp0 tasks finished"
