...
for n in $node; do
    ...
    ssh wn/bash -c 'rm -f $dm_tree_dir/build/load_keys.data'; exit;"
done
    ...
    for thread in $threads; do
        for file_name in $workloads; do
            ...
            for n in $node; do
                ...
                ssh we_dir/build; nohup ./ycsbc $thread 4 $file_name $dis > $dm_tree_dir/data/node$n-exp0_rolex_$file_name-$dis-thread$thread-coro4.txt 2>&1 &"
            done
            ...
            ssh wdm_tree_dir/script && bash kill_server.sh'; exit;"
            echo "============================="
            ...
            for n in $node; do
                ...
                scp "wnode$n-exp0_rolex_$file_name-$dis-thread$thread-coro4.txt" "$ae_data_dir/"
            done
            ...
            if [ "$file_name" = "scan-only" ]; then
                echo "Cleaning load_keys.data on compute nodes"
                for n in $node; do
                    ssh wtree_dir/build/load_keys.data'; exit;"
                done
                rm -f "$dm_tree_dir/build/load_keys.data"
            fi
