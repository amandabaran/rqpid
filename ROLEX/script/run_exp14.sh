...
for n in $node; do
    ...
    ssh w-c 'rm -f $dm_tree_dir/build/load_keys.data'; exit;"
done
    ...
    for thread in $threads; do
        for file_name in $workloads; do
            ...
            for n in $node; do
                ...
                ssh wuild; nohup ./ycsbc $thread 4 $file_name $dis > $dm_tree_dir/data/node$n-exp0_rolex_$file_name-$dis-thread$thread-coro4.txt 2>&1 &"
            done
