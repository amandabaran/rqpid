...
for n in $memory_nodes; do
    ssh wn/bash -c 'sudo sysctl -w vm.nr_hugepages=0'; exit"
done

for n in $node; do
    ssh w-c 'sudo sysctl -w vm.nr_hugepages=0'; exit"
done
...
for dis in $distribution; do
    for thread in $threads; do
        for file_name in $workloads; do
            ...
            ssh wdm_tree_dir/script && bash restart_memc.sh'; exit;"
            ssh wc 'cd $dm_tree_dir/script && bash run_server.sh'; exit;"
            echo "============================="
            ...
            for n in $node; do
                ...
                ssh w$n "cd $dm_tree_dir/build; nohup ./ycsbc $thread 4 $file_name $dis > $dm_tree_dir/data/node$n-exp0_dlsm_$file_name-$dis-thread$thread-coro4.txt 2>&1 &"
            done
            ...
            for n in $node; do
                ...
                scp "wtree_dir/data/node$n-exp0_dlsm_$file_name-$dis-thread$thread-coro4.txt" "$ae_data_dir/"
            done
