...
for n in $memory_nodes; do
    ssh wn/bash -c 'sudo sysctl -w vm.nr_hugepages=68768'; exit"
done
...
for dis in $distribution; do
    for thread in $threads; do
        for file_name in $workloads; do
            ...
            ssh w3 "/bin/bash -c 'cd $dm_tree_dir/script && bash restart_memc.sh'; exit;"
            ssh w3 "/bin/bash -c 'cd $dm_tree_dir/script && bash run_server.sh'; exit;"
            echo "============================="
            ...
            for n in $node; do
                ...
                ssh w$n "cd $dm_tree_dir/build; nohup ./ycsbc $thread 4 $file_name $dis > $dm_tree_dir/data/node$n-exp0_chime_$file_name-$dis-thread$thread-coro4.txt 2>&1 &"
            done
            
            sleep 2
            echo "Running ycsbc on compute node 7"
            cd $dm_tree_dir/build && ./ycsbc $thread 4 $file_name $dis > $dm_tree_dir/data/node7-exp0_chime_$file_name-$dis-thread$thread-coro4.txt 2>&1;
            echo "============================="
            
            # Run kill_server.sh script on memory nodes
            echo "Kill server process on memory nodes"
            ssh w3 "/bin/bash -c 'cd $dm_tree_dir/script && bash kill_server.sh'; exit;"
            echo "============================="
            ...
            for n in $node; do
                ...
                scp "w$n:$dm_tree_dir/data/node$n-exp0_chime_$file_name-$dis-thread$thread-coro4.txt" "$ae_data_dir/"
            done
            echo "Copying local data from node7 to node7's AE/Data directory"
            cp "$dm_tree_dir/data/node7-exp0_chime_$file_name-$dis-thread$thread-coro4.txt" "$ae_data_dir/"
            echo "============================="
...
echo "============================="
echo "All CHIME tasks are finished"
