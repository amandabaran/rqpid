    ...
    for thread in $threads; do
        for file_name in $workloads; do
            ...
            for n in $node; do
                ...
                ssh w$n "cd $dm_tree_dir/build; nohup ./ycsbc $thread 4 $file_name $dis > $dm_tree_dir/data/node$n-exp0_fptree_$file_name-$dis-thread$thread-coro4.txt 2>&1 &"
            done
            
            sleep 2
            echo "Running ycsbc on compute node 7"
            cd $dm_tree_dir/build && ./ycsbc $thread 4 $file_name $dis > $dm_tree_dir/data/node7-exp0_fptree_$file_name-$dis-thread$thread-coro4.txt 2>&1;
            echo "============================="
...
echo "============================="
echo "All FPTree tasks are finished"
