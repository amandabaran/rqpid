...
for dis in $distribution; do
    for thread in $threads; do
        for file_name in $workloads; do
            ...
            for n in $node; do
                sleep 2
                ssh w$n "cd $base_dir/build && nohup numactl --interleave=all ./ycsbc $thread 4 $file_name $dis > $base_dir/data/node$n-exp0_smart_$file_name-$dis-thread$thread-coro4.txt 2>&1" &
            done

            sleep 2
            cd $base_dir/build && numactl --interleave=all ./ycsbc $thread 4 $file_name $dis \
                > $base_dir/data/node$driver-exp0_smart_$file_name-$dis-thread$thread-coro4.txt 2>&1
            ...
            for n in $node; do
                scp "w$n:$base_dir/data/node$n-exp0_smart_$file_name-$dis-thread$thread-coro4.txt" "$ae_data_dir/"
            done
            cp "$base_dir/data/node$driver-exp0_smart_$file_name-$dis-thread$thread-coro4.txt" "$ae_data_dir/"
...
echo "All SMART exp0 tasks finished"
