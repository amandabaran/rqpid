...
for n in $memory_nodes; do
    ssh wn/bash -c 'sudo sysctl -w vm.nr_hugepages=82768'; exit"
done

# Reduce hugepage count on compute nodes due to SMART's high temporary memory usage.
for n in $node; do
    ssh w$n "/bin/bash -c 'sudo sysctl -w vm.nr_hugepages=3268'; exit"
done
    ...
    for thread in $threads; do
        for file_name in $workloads; do
            ...
            for n in $node; do
                ...
                ssh w$n "cd $dm_tree_dir/build; nohup ./ycsbc $thread 4 $file_name $dis > $dm_tree_dir/data/node$n-exp0_smart_$file_name-$dis-thread$thread-coro4.txt 2>&1 &"
            done
            ...
            for n in $node; do
                ...
                scp "w$n:$dm_tree_dir/data/node$n-exp0_smart_$file_name-$dis-thread$thread-coro4.txt" "$ae_data_dir/"
            done
