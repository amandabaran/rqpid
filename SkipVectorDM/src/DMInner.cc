...
// init root node
void SkipVectorDM::init_root_node() {
	...
	auto root_fp_addr = dmv->compute_alloc(fp_table->get_size());
...
// get root addr
void SkipVectorDM::get_root_addr(Gaddr& root_addr, int coro_id, CoroContext* ctx) {
...
// search inner index
bool SkipVectorDM::inner_search(Gaddr addr, const Key& key, Record& record,
...
// find to leaf
Record SkipVectorDM::find_leaf(const Key& key, int coro_id, CoroContext* ctx) {
...
bool SkipVectorDM::bottom_search(Gaddr addr, const Key& key, Record& record,
                           bool& next_is_leaf, int coro_id, CoroContext* ctx) {
...
Record SkipVectorDM::read_bottom_inner(const Key& key, int coro_id,
                                 CoroContext* ctx) {
...
// lock inner node
void SkipVectorDM::lock_node(Gaddr addr, uint64_t lock_offset, int coro_id,
                       CoroContext* ctx) {
...
// unlock inner node
void SkipVectorDM::unlock_node(Gaddr addr, uint64_t lock_offset, int coro_id,
                         CoroContext* ctx) {
...
// index insertion
bool SkipVectorDM::inner_write(Gaddr &addr, Record& inner_record,
...
// inerternal split
void SkipVectorDM::inner_insert(const Key& key, Record& inner_record,
...
// block split
void SkipVectorDM::node_split(Gaddr addr, Record& record, int coro_id,
                        CoroContext* ctx) {
...
// update root index
void SkipVectorDM::update_root(Record& record, Gaddr update_addr, int coro_id,
                         CoroContext* ctx) {
