...
thread_local InnerTree SkipVectorDM::inner_tree[define::kMaxCoro];
thread_local LeafNode SkipVectorDM::leaf_nodes[define::kMaxCoro];
thread_local FPTable SkipVectorDM::fp_tables[define::kMaxCoro];

void SkipVectorDM::leaf_search(Gaddr addr, Record& path, bool& from_cache,
	...
	bool from_local_fp = true;
    bool is_sync_leaf = false;

leaf_search_reread:
	// read the fp locally
	auto local_fp_buffer = dmcache->get_local_fp_buffer(path.fp_addr);
    char tmp_fp_buffer[128];
    memcpy(tmp_fp_buffer, local_fp_buffer, 64);
	fp_table->attach(tmp_fp_buffer, path.capacity);

	// if not cached in local, read the fp from the compute side
	if(fp_table->fp[0] == 0 || !from_local_fp || path.fp_addr.nodeID == 0) {
		from_local_fp = false;
		fp_table->attach(fp_buffer, path.capacity);
	...
	if(!from_local_fp && path.fp_addr.nodeID != dmv->getMyNodeID() && path.fp_addr.nodeID != 0) {
		memcpy(local_fp_buffer, fp_buffer, fp_table->get_size());
	}
...
// lock entry
void SkipVectorDM::lock_entry(Gaddr addr, uint64_t lock_offset, int coro_id,
                        CoroContext* ctx) {
...
// unlock entry
void SkipVectorDM::unlock_entry(Gaddr addr, uint64_t lock_offset, int coro_id,
                          CoroContext* ctx) {
...
LeafHeader SkipVectorDM::get_leaf_hdr(Gaddr addr, int coro_id, CoroContext* ctx) {
...
// lock entry
void SkipVectorDM::lock_table(Gaddr addr, uint64_t lock_offset, int coro_id,
                        CoroContext* ctx) {
...
void SkipVectorDM::leaf_insert(Gaddr& addr, Record& path, bool& from_cache,
                         const Key& key, Value& value, int coro_id,
                         CoroContext* ctx) {
...
bool SkipVectorDM::try_lock_table(Gaddr addr, uint64_t lock_offset, int coro_id,
                            CoroContext* ctx) {
...
void SkipVectorDM::unlock_table(Gaddr addr, uint64_t lock_offset, int coro_id,
                          CoroContext* ctx) {
...
bool SkipVectorDM::leaf_resize(Gaddr addr, Gaddr fp_addr, const Key& key,
	...
	Gaddr sibling_fp_addr = dmv->compute_alloc(sib_fp_table->get_size());
...
bool SkipVectorDM::read_table(Gaddr addr, char* buffer, int entry_num, int coro_id,
                        CoroContext* ctx) {

	auto& leaf_node = leaf_nodes[coro_id];
	int read_num = entry_num;
	if(entry_num < leaf_node.cur_capacity()) {
		read_num = entry_num + 1;
	}

	// TODO: what if these is insert, update the node but not the fp_table
	auto node_used_size = sizeof(LeafHeader) + read_num * sizeof(Entry);
	dmv->read_sync(buffer, addr, node_used_size, ctx);
...
void SkipVectorDM::leaf_scan(Gaddr addr, Record& path, bool& from_cache,
    ...
    auto fp_buffer = buffer + leaf_node.cur_node_size();
    auto fp_table = &fp_tables[coro_id];
    fp_table->attach(fp_buffer, path.capacity);
	...
	while(addr != Gaddr::Null() && len > 0) {
		...
		if(head_node == true && fp_table->get_version() != path.version) {
		...
		auto entry_num = fp_table->get_entry_num();
...
bool SkipVectorDM::read(const Key& key, Value& value, int coro_id, CoroContext* ctx) {
...
void SkipVectorDM::write(const Key& key, Value& value, int coro_id,
                   CoroContext* ctx) {
	Gaddr leaf_addr = Gaddr::Null();
	Record inner_record;
...
int SkipVectorDM::scan(const Key& from, const Key& to, int len, int coro_id,
                 CoroContext* ctx) {
	Gaddr leaf_addr = Gaddr::Null();
	Record inner_record;
...
void SkipVectorDM::background_sort_meta(Gaddr addr, int capacity, int coro_id,
                                  CoroContext* ctx) {}