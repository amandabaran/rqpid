...
#define USE_CORO
#define SkipVectorDM_LATENCY
#define SkipVectorDM_MAX_LATENCY_SIZE 100000
...
uint64_t dis_latency[MAX_THREAD_NUM][define::kMaxCoro][SkipVectorDM_MAX_LATENCY_SIZE];
#endif
...
void do_transaction() {
	...
	if(record.type == util::INSERT) {
		Value value = 12;
		skipvecdm->write(str2key(key), value);
	}
	if(record.type == util::READ) {
		Value value;
		skipvecdm->read(str2key(key), value);
	}
	...
	if(record.type == util::SCAN) {
		...
		Value values[1000];
		skipvecdm->scan(str2key(key), KeyMax, record.scan_len);
	}
	...
	if(exeTime[th_id][0] / 100 >= SkipVectorDM_MAX_LATENCY_SIZE) {
...
void coro_worker(CoroYield& yield, uint64_t operation_count,
	...
	while(thread_finish_ops < operation_count) {
		...
		if(record.type == util::INSERT) {
			Value value = 12;
			skipvecdm->write(str2key(key), value, coro_id, &ctx);
		}
		if(record.type == util::READ) {
			Value value;
			skipvecdm->read(str2key(key), value, coro_id, &ctx);
		}
		if(record.type == util::UPDATE) {
			Value value = 12;
			skipvecdm->write(str2key(key), value, coro_id, &ctx);
		...
		if(record.type == util::SCAN) {
			std::vector<std::string> vec;
			Value values[1000];
			skipvecdm->scan(str2key(key), KeyMax, record.scan_len, coro_id, &ctx);
		}
		if(record.type == util::READMODIFYWRITE) {
			Value value;
			value = 12;
		...
		if(exeTime[th_id][coro_id] / 100 >= SkipVectorDM_MAX_LATENCY_SIZE) {
...
int main(const int argc, const char* argv[]) {
	...
	dmv->registerThread();
	skipvecdm = new SkipVectorDM(dmv);
	...
	uint64_t perload_ops = 200000000;
...
#ifdef SkipVectorDM_LATENCY
	int lat_count = 0;
	for(int i = 0; i < SkipVectorDM_MAX_LATENCY_SIZE; i++) {
